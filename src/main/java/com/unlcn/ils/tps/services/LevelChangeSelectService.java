 ...
 * 
 *@Title:
 *@Description:级别调整自定义数据源 
 *@Author:johnn
 *@Since:2016-8-18
 *@Version:1.1.0
 */
public class LevelChangeSelectService implements ArrayContentProvider{
	...
	public Page<Map<String,String>> getElements(ArrayContext context) {
		...
		for(int i=0;i<lists.size();i++){
			...
			//if(e_currentlevel.getActive()!=0){//0为禁用 ，1为可用
			...
			logger.info("长度为 "+levelchanges.size());
